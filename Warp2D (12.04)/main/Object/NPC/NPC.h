...
public:
	...
	// 상점을 띄우자
	// 플레이어가 필요한 이유 (플레이어의 돈을 얼마나 가지고 있는지)
	virtual void Interaction(CScene* pScene);
...
private:
	...
	ComPtr<ID2D1SolidColorBrush> playerGrayBrush;
...
public:
	...
	const D2D_SIZE_U GetAttCoord() const
	{
		...
		switch (m_Direction)
		{
		case CNPC::left: if (retval.width > 0)	retval.width -= 1;	break;
		case CNPC::top:	if (retval.height > 0)	retval.height -= 1;	break;
		case CNPC::right:						retval.width += 1;	break;
		case CNPC::bottom:						retval.height += 1;	break;
		}
		return retval;
	}
