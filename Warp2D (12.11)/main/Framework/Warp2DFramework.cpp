...
void CWarp2DFramework::BindKey()
{
	...
	/*
		GetPrivateProfileIntA (int 값으로 읽어달라 string 버전도 있다. 찾아봐라)
		[Main] 앱
		"exit" 키
		VK_ESCAPE 못 읽으면 결과값이 27이 된다. (ini파일) 
		config.ini 파일에서 읽는다.
	*/
...
{
	// 전부 false로 만듦
	// 상위 레이어에서 처리가 되었는가에 대한 판단 ( SceneTitle의 SpaceBar와 SceneMain의 SpaceBar의 기능이 다르기 때문에)
	bool arrByte[256]{};
	memset(arrByte, false, sizeof BYTE * 256);

	// 현재 돌아가는 Scene의 ProcessInput
	if (m_pCurrentScene) m_pCurrentScene->ProcessInput(arrByte);

	// 끝나면 실행
	// ProcessInput 특정 개체가 들고 있는 것
