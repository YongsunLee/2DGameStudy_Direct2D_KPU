...
bool CTestScene::OnCreate(wstring && tag, CWarp2DFramework * pFramework)
{
	...
	{
		retryEquipItem:
		auto item = make_unique<CItemEquip>(SizeU(pos_random(reng), pos_random(reng)));
		for (const auto& p : m_listEquipItem) if (p->IsCollision(item->GetCoord())) goto retryEquipItem;
		...
		item->RegisterImage(m_pIndRes.get(), rendertarget.Get(), imgPath);
		m_listEquipItem.push_back(move(item));
	}
	*/
...
void CTestScene::Draw(ID2D1HwndRenderTarget * pd2dRenderTarget)
{
	...
	for (const auto& p : m_lstItem)			p->Draw(pd2dRenderTarget);
	//for (const auto& p : m_listEquipItem)	p->Draw(pd2dRenderTarget);
	for (const auto& p : m_lstTrap)			p->Draw(pd2dRenderTarget);
